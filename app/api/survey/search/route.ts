...
export async function POST(request: NextRequest) {
  try {
    ...
    const { query, categoryId, componentKey, limit = 10, threshold = 0.3 } = body;
    ...
    // Call match_survey_items RPC
    const { data: results, error: searchError } = await supabase.rpc("match_survey_items", {
