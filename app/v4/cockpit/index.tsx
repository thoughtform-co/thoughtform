...
// ═══════════════════════════════════════════════════════════════════
// v4: R3F-based particle field replaces the 2D ParticleCanvasV2.
const ParticleFieldR3F = dynamic(
  () => import("@/components/hud/r3f/ParticleFieldR3F").then((m) => m.ParticleFieldR3F),
  {
    ssr: false,
    loading: () => null,
  }
...
const ThreeGateway = dynamic(
  () => import("@/components/hud/ThreeGateway").then((m) => m.ThreeGateway),
  {
    ssr: false,
    loading: () => null,
  }
...
function NavigationCockpitV4Inner() {
  ...
  return (
    <>
      ...
      {/* v4: R3F particle field replacing the 2D ParticleCanvasV2. */}
      <CanvasErrorBoundary>
        <ParticleFieldR3F scrollProgress={scrollProgress} />
      </CanvasErrorBoundary>
      ...
      {!isMobile && (
        <div
          style={{
            // v4: match the manifesto blur so the incoming services deck
            // softens with the outgoing panel during the transition band.
            filter: sceneTransition.active ? `blur(${sceneTransition.blur}px)` : undefined,
            transition: "filter 60ms linear",
            // Don't let the wrapper block interactions outside the band.
            pointerEvents: sceneTransition.active ? "none" : undefined,
          }}
        >
          <ServicesDeck
...
interface NavigationCockpitV4Props {
  /** Initial particle config from server-side fetch (prevents flash) */
  initialParticleConfig?: ParticleSystemConfig;
}

// Main component with provider wrapper
export function NavigationCockpitV4({ initialParticleConfig }: NavigationCockpitV4Props) {
  return (
    <ParticleSceneProvider>
      <ParticleConfigProvider initialConfig={initialParticleConfig}>
        <SigilConfigProvider>
          <NavigationCockpitV4Inner />
          {ENABLE_GATE && <GateOverlay />}
        </SigilConfigProvider>
