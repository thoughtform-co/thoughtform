...
- **`getMobilePaintProgress(progress)`** — a re-export of `phonePaintProgress` from `lib/home-v2/phoneCorridorClock.ts` (ADR-125, U1): a monotonic, velocity-continuous LEG TABLE in svh of scroll from the pin — a 40svh thesis hold at paint 0, a 60svh rise to `dollyHoldEnd`, then a `cruise` pass into each park (the three passes derived to ONE speed, 1.333× the desktop's, sine ramps over a quarter of each and a linear middle) and a 24svh DWELL on it (Navigate / Encode / Build held at `BEAT_PARK_CENTRES` by reference), a 40svh linear tail to paint 1. `MOBILE_THOUGHTFORM_END` is derived from the same table (100svh / the corridor's 544.39svh). Four `normal` snap seats (`CorridorPhoneSeats`) sit on the dwells; the beats are composed on the chrome bands by straddles the tracker DERIVES per frame height (`lib/home-v2/phoneStraddle.ts`), the sphere solved from the tightest band. Applied at the `paintProgress` write in `useDepthScroll.ts`; on mobile `beat`/`gateProgress` are derived from the painted value, not raw progress. ⚠ The follower's teleport detector reads RAW progress for the same reason.
