...
export function ImageParticleGateway({
  ...
  const IMAGE_PARTICLES_END = 0.3; // When image particles completely disappear
      ...
      for (let py = 0; py < sampleHeight; py += step) {
        for (let px = 0; px < sampleWidth; px += step) {
          ...
          if (distFromCenter < 0.22) {
            ...
            const angle = Math.atan2(py / sampleHeight - 0.45, px / sampleWidth - 0.55);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const transitionP = Math.max(
        0,
        Math.min(1, (scrollP - TRANSITION_START) / (TRANSITION_END - TRANSITION_START))
      );

      // Calculate fade-out progress for image particles
      const imageParticleFade =
        scrollP < TRANSITION_END
          ? 1
        ...
        sorted.forEach((p) => {
          ...
          if (p.finalType === "gateway" && scale > 0.3) {
            ...
            drawPixel(
              ...
              screenY,
              GOLD_RGB,
              finalAlpha * 0.2 * glowPulse * glowIntensity,
              size * 4
            );
          }
        });
      ...
      if (terrainParticlesRef.current.length > 0) {
        ...
        const baseTerrainAlpha = 0.2; // Subtle at start
        const growthAlpha = scrollP < 0.05 ? 0 : Math.min(0.8, (scrollP - 0.05) / 0.15); // Full by 20% scroll
        ...
        terrainParticlesRef.current.forEach((p) => {
          ...
          const finalAlpha =
            depthAlpha * breatheAlphaVal * terrainAlpha * terrainOpacity * proximityBoost;
