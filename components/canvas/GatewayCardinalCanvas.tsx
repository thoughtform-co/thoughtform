...
export function GatewayCardinalCanvas() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      annotations.push({
        start: { x: Math.cos(angle) * startR, y: Math.sin(angle) * startR * 0.3, z: Math.sin(angle) * 0.5 },
        end: { x: Math.cos(angle) * endR, y: Math.sin(angle) * endR * 0.3 + (Math.random() - 0.5) * 0.5, z: Math.sin(angle) * 0.5 + (Math.random() - 0.5) * 0.5 },
    ...
    function drawPixel(x: number, y: number, color: [number, number, number], alpha: number, size = gridSize) {
    ...
    function drawLine(x1: number, y1: number, x2: number, y2: number, color: [number, number, number], alpha: number, width = 1) {
    ...
    function draw() {
      ...
      for (const ann of annotations) {
        ...
        if (ann.hasTick) {
          ...
          drawLine(mx - Math.cos(angle) * 4, my - Math.sin(angle) * 4, mx + Math.cos(angle) * 4, my + Math.sin(angle) * 4, colors.dawn, 0.18, 0.5);
        }
      }
      ...
      for (let i = 0; i < 35; i++) {
        ...
        const r = 0.18;
        const sp = projectGateway(4.5 + r * Math.sin(angle2) * Math.cos(angle1), -2.2 + r * Math.sin(angle2) * Math.sin(angle1), -2 + r * Math.cos(angle2));
