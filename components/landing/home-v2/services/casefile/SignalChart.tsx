...
import type { CaseSignalPoint } from "@/lib/cases/types";
...
function curvePath(points: readonly CaseSignalPoint[]): string {
...
interface SignalChartProps {
  points: readonly CaseSignalPoint[];
  t0: string;
  now: string;
}
    ...
    >
      <defs>
        <clipPath id={clipId} clipPathUnits="userSpaceOnUse">
          <rect className="fl-signal__wipe" x={0} y={0} width={VB_W} height={VB_H} />
        </clipPath>
      </defs>
      ...
      <g clipPath={`url(#${clipId})`}>
        <path className="fl-signal__halo" d={d} />
        <path className="fl-signal__line" d={d} />
        {points.map((pt, i) => {
          const x = px(pt.x);
          const y = py(pt.y);
          ...
          return (
            ...
            <g key={i}>
              ...
              />
              <text className="fl-signal__stamp" x={tx} y={stampY} textAnchor={anchor}>
                {pt.stamp}
              </text>
              <text className="fl-signal__caption" x={tx} y={stampY + 12} textAnchor={anchor}>
                {pt.label}
              </text>
            </g>
          );
      ...
      <text className="fl-signal__foot" x={PAD.l - 14} y={VB_H - 10}>
        {t0}
      </text>
      <text
        className="fl-signal__foot fl-signal__foot--gold"
        x={VB_W - PAD.r + 14}
        y={VB_H - 10}
        textAnchor="end"
      >
