...
const CROP = SHEET_VIEWBOX.board;
 ...
 *
 * THE PARTS INDEX IS NOT ON THIS SHEET, and that is a measurement, not a
 * preference. A schematic without its index is decor — but 27 index rows at
 * a readable size need ~700 units of height in a crop that has 570, and
 * squeezing both into the casefile's 611px console put every label at
 * 6.8px. The index survives in the three places that have room for it: the
 * hover card names any module on demand, the mobile fallback IS the index,
 * and the expanded view restores the authored left-third schedule.
 *
 * ACCESSIBILITY. With the index gone the chips ARE the control, so they
 * carry the focus ring and the label rather than being `aria-hidden` twins.
 */
...
export function MapSheetBoard({
  ...
  return (
    <g>
      <text className="fl-imap__t fl-imap__t--gold" x={CROP.x + 8} y={CROP.y + 22}>
        Sheet 01 / the board — the work
      </text>
      <text className="fl-imap__t" x={CROP.x + 8} y={CROP.y + 40}>
      ...
      <g>
        ...
        {Array.from({ length: 11 }, (_, i) => {
          ...
          return (
            <line className="fl-imap__board-grid" key={`gx${i}`} x1={p1[0]} y1={p1[1]} x2={p2[0]} y2={p2[1]} />
          );
        })}
        {Array.from({ length: 7 }, (_, j) => {
          ...
          return (
            <line className="fl-imap__board-grid" key={`gy${j}`} x1={p1[0]} y1={p1[1]} x2={p2[0]} y2={p2[1]} />
          );
        })}
      </g>
      ...
      <g>
        ...
        <line
          ...
          y2={busR[1] + 1.5}
        />
        <text className="fl-imap__t fl-imap__t--gold" x={busL[0] - 8} y={busL[1] - 4} textAnchor="end">
          The bus
        </text>
        <text className="fl-imap__t fl-imap__t--faint" x={busL[0] - 8} y={busL[1] + 7} textAnchor="end">
          One standard
        </text>
      </g>
      ...
      {placed.map((p) => {
        ...
        const traceFrom = iso(
          ...
          BOARD.cy,
          p.a,
          p.b > 0 ? p.b - BOARD.db : p.b + BOARD.db
        );
      ...
      <g>
        {placed.map((p) => {
          ...
          const w = p.district.name.length * 9.4 + 16;
