...
- **Proof / client cases (the evidence beat):** [ADR-054](sentinel/decisions/054-proof-station-client-cases.md) — `#proof` (the Loop Earplugs case) REPLACES the ADR-049 continuum in the funnel: a mission report + three zig-zag beats mapped onto the Arc, mirroring THE ARC's menu subs without repeating its 3D. Plain opaque DOM, no flag; it is the ambient-kill cover after `#about`. Content model in `lib/cases/` (types-only + registry, the `lib/arcs` mirror), generated into the station at parse time by `lib/v7-parse/proofStation.ts` via `ParseOptions.fillSlots`. Rules in [`.claude/rules/proof.md`](.claude/rules/proof.md). "Case" ≠ "arc page" ≠ `arc-cases/` (LANGUAGE.md). ADR-049 is superseded (the `uBand*` shader block stays dormant at 0 gain)
...
- **Client arcs (/arcs):** [ADR-052](sentinel/decisions/052-client-arcs.md) — client deck pages (workshop/keynote) on the HUD slice; content model in `lib/arcs/`, rules in [`.claude/rules/arcs.md`](.claude/rules/arcs.md). "Arc page" ≠ "the Arc" (LANGUAGE.md)
