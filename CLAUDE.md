...
- **Rail instruments + the four-corner scheme:** [ADR-059](sentinel/decisions/059-rail-instruments.md) — journey (all ten section marks, one row) TOP-LEFT, nav TOP-RIGHT, brand BOTTOM-LEFT, settings BOTTOM-RIGHT; plus the right rail's bearing/sector/local telemetry. Flags `RAIL_INSTRUMENTS` / `SETTINGS_CLUSTER`. ⚠ two clocks in one row (beats on the manifest index, `proof` on the readout index); look-dev at `/test/hud-instruments-lab`
