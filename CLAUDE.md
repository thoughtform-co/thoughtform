...
- **The casefile's right panel is ONE instrument:** [ADR-064](sentinel/decisions/064-casefile-console-frame.md) — all four evidence plates render inside the shared `ConsoleFrame` (`casefile/console/**`), so the panel changes what it displays rather than being four boxes in a slot. ⚠ The frame is a BEZEL THE CONTENT BLEEDS INTO, never a letterbox; each plate keeps its own interior (the rails are deliberately not unified); below 980px the console UNWRAPS rather than hiding. ⚠ **NO FILTER ON THE EVIDENCE** — ADR-056 U5 upheld: the uniformity is the chrome, the stills/films are the client's own creative
