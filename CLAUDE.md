...
- **Rail instruments + the four-corner scheme:** [ADR-059](sentinel/decisions/059-rail-instruments.md) — approach marks TOP-LEFT, nav TOP-RIGHT, brand BOTTOM-LEFT, destination marks + settings on ONE LINE BOTTOM-RIGHT (U2); plus the right rail's bearing/sector/local telemetry. Flags `RAIL_INSTRUMENTS` / `SETTINGS_CLUSTER`. ⚠ two clocks (beats on the manifest index, `proof` on the readout index), and the bottom-right row's curtain clip is a COPY of `.hud__corner--br`'s; look-dev at `/test/hud-instruments-lab`
