...
- **Rail instruments:** [ADR-059](sentinel/decisions/059-rail-instruments.md) — the journey's sections as marks in the TOP-LEFT and BOTTOM-RIGHT corners (each replacing that corner's bracket), plus the right rail's bearing/sector/local telemetry. Flag `RAIL_INSTRUMENTS`. ⚠ moves the ADR-058 theme toggle to the bottom-LEFT; look-dev at `/test/hud-instruments-lab`
