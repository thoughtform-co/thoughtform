/**
 * shapes — canonical brandmark path variants.
 *
 * The brandmark is one artifact that can sample to different shape
 * topologies depending on which keyframe the journey transform is
 * parked at. Two variants today:
 *
 *   - `full`: the canonical Thoughtform brandmark — outer arc +
 *     interior cross + horizontal bar + small flourishes. Used by
 *     every keyframe except substrate.
 *   - `ring`: the outer arc only — paths 0 and 1 of the source SVG,
 *     which together form the C-arc that encloses the rest of the
 *     mark. When the painter blends to this set the cross + bar
 *     dissolve and the cloud reads as a thick gold orbital ring,
 *     so the brandmark literally BECOMES the middle circle of the
 *     intelligence-layer triad (ADR-014).
 *
 * The full path set lives here so `BrandmarkGlyph.tsx` and the
 * particle painter share one source of truth. The viewBox and the
 * outline asset URL stay in `BrandmarkGlyph` (they describe the
 * inline-SVG render contract, not the shape topology).
 */

export type BrandmarkShapeKey = "full" | "ring";

/**
 * Path geometry exported from `public/logos/Thoughtform_Brandmark.svg`.
 * viewBox is "0 0 430.99 436". Do not edit by hand — re-export from
 * the source SVG if the brandmark geometry changes.
 */
export const BRANDMARK_FULL_PATHS: readonly string[] = [
...
/**
 * The outer C-arc only. Paths 0 and 1 of the source SVG already
 * describe a closed thick-ring topology that hugs the brandmark's
 * outer edge. Sampled in isolation the resulting point cloud reads
 * as a clean orbital ring with no interior detail — ideal for the
 * substrate dock of the intelligence-layer triad.
 *
 * Path indices are preserved (0, 1) so the per-particle rank from
 * the full-mark sample lines up index-by-index with the ring sample
 * for the painter's `mix(aHome, aHomeRing, uShapeBlend)` lerp.
 */
export const BRANDMARK_RING_PATHS: readonly string[] = [
  BRANDMARK_FULL_PATHS[0],
  BRANDMARK_FULL_PATHS[1],
];

/**
 * BRANDMARK_SHAPES — the keyed shape table consumed by the painter
 * and the journey transform. Add a new entry here (with its own
 * stable key) when a future keyframe needs another topology.
 */
export const BRANDMARK_SHAPES: Record<BrandmarkShapeKey, readonly string[]> = {
  full: BRANDMARK_FULL_PATHS,
  ring: BRANDMARK_RING_PATHS,
};

/**
 * Stable cache key per shape variant. Passed to `sampleShape` so
 * each variant gets its own memoised sample buffer.
 */
export const BRANDMARK_SHAPE_KEYS: Record<BrandmarkShapeKey, string> = {
  full: "brandmark",
  ring: "brandmark-ring",
};
