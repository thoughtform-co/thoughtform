...
/** The ONE remote origin video may stream from: the site's own Supabase
 *  project, whose public `era-media` bucket holds the era stage's films (ADR-082
 *  U34). Mirrored by `ERA_MEDIA_STORAGE_ORIGIN` in the zero-import era registry,
 *  and `tests/lib/security-headers.test.ts` asserts the two are one string. */
export const ERA_MEDIA_ORIGIN = "https://ehijwavsxbvnxsrunegu.supabase.co";
...
export function buildContentSecurityPolicy(options = {}) {
  const directives = {
    ...
    /* ⚠ SELF-HOSTED, PLUS ONE NAMED BUCKET — AND THE BUCKET IS A DECISION.
       Every film and walkthrough on this site lives in `public/videos/`
       because of this line — a remote `src` is blocked under the enforced
       CSP, so video can never QUIETLY move to a bucket. ADR-082 U34 moved
       ONE on purpose (owner, 2026-09-22: the Twitch recording of his World
       of Warcraft class, "add that to Supabase", and asked, "stream from
       Supabase"): `ERA_MEDIA_ORIGIN`, the site's own project, named in full.
       ⚠ NEVER the `*.supabase.co` glob that `img-src` and `connect-src`
       carry: that would let any project on the platform serve video here.
       Pinned from both directions by `tests/lib/security-headers.test.ts`.
       ...
       not a remote host. */
    "media-src": ["'self'", "blob:", "data:", ERA_MEDIA_ORIGIN],
