...
// (ADR-042) `SIGIL_Z` — the shell-local Z of the sphere-welded cases sigil — is
// retired along with the sigil itself. The trigger is now `ArcCasesCue`, a DOM
// label docked under the Build title, so it has no place on the sphere's optical
// axis. The in-canvas card (`CARD_Z`) is unchanged.
