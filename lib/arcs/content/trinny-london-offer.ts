 ...
 *
 * ⚠ IT LIVES IN `lib/arcs/content/` SINCE ADR-118 U2, and the route's old
 * path (`offer/offerSections.ts`) re-exports it: the `/arcs` overview draws
 * the client's configuration from `TRINNY_BOARD`'s tools and
 * `TRINNY_PHASES`' plates, and a `lib/` module may not import from `app/`.
 * Still pure data, still outside `ARCS` — nothing under `lib/arcs` walks the
 * content directory, so a module here that is not an arc is inert.
 */
/**
