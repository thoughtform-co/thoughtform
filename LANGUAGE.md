...
| **Configuration board** | The log's dossier's drawing of a client's intelligence configuration (ADR-118 U2): one ink DIE of the workstreams the proposal names, CHIPS for what it runs on and inside, BUSES between them. Derived from the proposal's own record, never authored; proposals only. Not the landing's map reading 02 ("the configuration", ADR-070), and not the Trinny page's `board` kind (ADR-100). |
