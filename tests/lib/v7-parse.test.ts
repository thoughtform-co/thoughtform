 ...
 *     buildQuote · build · tools (the last two retired with ADR-033 — the
...
describe("v7-parse — production homepage station surgery (ADR-018, ADR-021)", () => {
  ...
  it("strips matching #hudNav entries when their station is removed", () => {
    ...
    // The hero's "Begin navigation" CTA — it has aimed at #continuum, then
    // #proof, and now at #services, where the client casefile opens the
    // runway (ADR-056). It must point at a SURVIVING station: this stripper
    // removes any anchor to a removed one, so aiming it at a
    // corridor-replaced station would silently delete the button.
    expect(bodyHtml).toMatch(/<a\s[^>]*href="#services"/);
    expect(bodyHtml).not.toMatch(/<a\s[^>]*href="#proof"/);
  });
  ...
  it("relocates #services then #about after the mount and drops the trailing connector slot", () => {
    ...
    // #proof retired with ADR-056 — the station AND its generated-body
    // shell go with it, along with the earlier continuum station.
    expect(bodyHtml).not.toMatch(/<section\b[^>]*\bid="proof"/);
    expect(bodyHtml).not.toMatch(/<div\b[^>]*\bdata-proof-body/);
...
/* (The ADR-054 `#proof` station-generation block was deleted with ADR-056:
   `lib/v7-parse/proofStation.ts` is gone and the client case is a mounted
   component inside `ServicesStage`. `ParseOptions.fillSlots` survives as a
   general parse capability with no caller today — the contract it needs is
   covered by `fillStationSlot`'s own behaviour, and re-adding a consumer
   should re-add a test with it.) */
