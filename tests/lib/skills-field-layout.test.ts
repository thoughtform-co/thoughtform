...
const layoutFor = (projection: Projection) =>
  projectField({
    ...
    tiers: visual?.intelligence?.tiers,
    projection,
  });
...
describe("skills field layout (ADR-056 U17)", () => {
  it("the fixture is the live map track", () => {
    expect(visual, "no registry track with skills found").not.toBeNull();
    expect(placed.length).toBeGreaterThan(0);
    expect(visual?.intelligence?.tiers.length).toBe(4);
    expect(visual?.teamDraw?.length).toBeGreaterThan(0);
  });
  ...
  it("allocation clusters by tier lean, heads span their columns, rows never exceed the template", () => {
    ...
    const tiers = visual?.intelligence?.tiers ?? [];
    ...
    const tierByTeam = new Map((visual?.teamDraw ?? []).map((t) => [t.team, t.tier]));
