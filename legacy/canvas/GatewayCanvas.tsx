...
export function GatewayCanvas() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numAnnotations; i++) {
      const angle = (i / numAnnotations) * Math.PI * 2 + Math.random() * 0.3;
      ...
      const endY = Math.sin(angle) * endR * 0.3 + (Math.random() - 0.5) * 0.5;
    ...
    function draw() {
      ...
      for (const p of sortedTorus) {
        ...
        const lightAngle = Math.cos(p.theta + angleYRef.current) * 0.3 + 0.7;
        alpha *= lightAngle;
