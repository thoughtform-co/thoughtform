  ...
  useFrame((state) => {
    if (ref.current) {
      ...
      for (let i = 0; i < positions.count; i++) {
        ...
        positions.setZ(i, Math.sin(x * 0.5 + time * 0.5) * Math.cos(y * 0.5 + time * 0.3) * 0.5);
      }
  ...
  useFrame((state) => {
    if (ref.current) {
      ...
      for (let i = 0; i < positions.count; i++) {
        ...
        const wave = Math.sin(x * 0.5 + time) * Math.sin(z * 0.5 + time * 0.8) * 0.5;
...
export function ThreeBackground({ preset, opacity = 0.5, customCode }: ThreeBackgroundProps) {
