...
z:3   .home-corridor-host   (the revealed second section)
        └ .home-v2-stage         (820svh; NEVER transformed → useDepthScroll rect intact)
          └ .home-v2-stage__sticky  (fixed during entry → frozen at viewport top)
```

`useLandingScroll` computes `defTop` (the corridor mount's viewport top, = `100vh − scrollY` until the stage reaches the top) and sets `html[data-corridor-entry]` while `defTop > 0.5`. `home-v2.css` makes `.home-v2-stage__sticky` **fixed** (`position: fixed; inset: 0`) only under that flag, so the parked frame sits frozen at viewport top while the hero lifts off it. The sticky cell's children (canvas + copy + brandmark) are positioned by `useWorldDomTracker` in viewport-projected coords relative to that cell, so freezing the cell at viewport `(0,0)` lands them at their correct final positions. At `scrollY >= 100vh` the flag clears and the cell's native sticky pin + the flythrough take over at the same `top: 0`.
...
- **Never transform `.home-v2-stage`.** `useDepthScroll` reads the stage's rect for ALL corridor timing; transforming the stage (or the host, or any wrapper containing it) desyncs `progress` / `paintProgress` / `epilogueProgress` / `dockProgress`. The entry hold lives on `.home-v2-stage__sticky`.
- **The entry fixed hold is gated to `html[data-corridor-entry]`.** It MUST be absent outside the first-viewport band — especially during the ADR-021 docked exit, where `.home-v2-stage__canvas` becomes `position: fixed`. Verified `canvasPos: fixed` + sticky `transform: none` across the dock window.
- **No `--corridor-pin` counter-transform.** It was retired because sticky + transform fought under real wheel/trackpad scroll and read as a bounce. Use the fixed hold.
