...
**Current production substrate (2026-06-06, ADR-018):** after comparing the low-poly brain against the Shell variant in `/test/intelligence-artifact`, the homepage substrate returned to the Shell variant's cleaner read: one outer gold geodesic icosphere (`buildGeodesicEdges(SUBSTRATE_CAGE_RADIUS, 1)`) and NO inner dawn / white shell. `SUBSTRATE_CAGE_RADIUS` is `0.42`, tightened from `0.70` so the shell is proportionate to the persistent DOM/SVG brandmark (closer to the lab Shell ratio) rather than the retired 0.55 particle sphere. The low-poly brain remains in the lab variants only. Production `ShellSubstrate.tsx` uses `shellWrapEmerge` so the gold shell starts large, already surrounding the mark, and contracts inward to final radius — it never scales up from a point at the centre.
...
| Tune shell layer radii (substrate geodesic / orbit table / outer surfaces)                              | `components/landing/home-v2/DepthGatewayScene/shell/shellGeom.ts` — `SUBSTRATE_CAGE_RADIUS` (0.42), `SHELL_ORBITS`, `SURFACES_OUTER_RADIUS` (1.85). The inner dawn geodesic was removed.                                                                                                                          |
...
The substrate geodesic emerges via `shellWrapEmerge(reveal)` (starts large, contracts inward, opacity presence ramp to avoid pop). The source orbits + surfaces ports emerge via the current fold / stagger helpers (`foldEmerge(petalStagger(...))` where applied). All three layers persist so the assembled shell stays present at the Build landing around the persistent DOM brandmark. The particle substrate morph was removed from the production home page; the 2D SVG mark remains consistent across Navigate, Encode, and Build.
