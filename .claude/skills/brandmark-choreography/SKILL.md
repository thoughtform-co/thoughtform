...
**Petal-unfold motion (2026-06-05 revision):** each layer's individual elements (12 dodecahedron faces, 6 source orbits, 6 surfaces ports) are wrapped in their own sub-groups that start COLLAPSED AT THE BRAND MARK CENTER (position 0, scale 0) and unfold OUTWARD to their final positions with staggered timing. Reads as origami petals opening around the mark at each phase arrival, not a uniform scale-up from a single point. Per-element stagger uses `petalStagger(reveal, idx, total, overlap)`; per-element position+scale lerp uses `petalEmerge(stagger)`. Both helpers live in `shell/shellGeom.ts`.
...
| Tune the petal cascade (how staggered the elements unfold inside the parent window)                     | `shell/shellGeom.ts` — `petalStagger`'s `overlap` arg (0 = strict round-robin, 1 = uniform burst). Tuned per layer in the layer components (`SUBSTRATE_FACE_OVERLAP` 0.55, `SOURCES_ORBIT_OVERLAP` 0.6, `SURFACES_PORT_OVERLAP` 0.55).                                                             |
...
| Decompose a different polyhedron into per-face sub-groups                                               | `buildDodecahedronFaces(radius)` in `shell/shellGeom.ts` is the template — clusters `THREE.DodecahedronGeometry`'s triangles by face normal + angle-sorts the pentagon vertices                                                                                                                    |

Layers emerge **geometrically via per-element scale + position lerp** (Principle 4 — never opacity) on their staggered reveal and **persist** so the assembled shell stays present at the Build landing while `SubstrateMorphCloud` morphs the brandmark silhouette into the substrate sphere at the centre.

**`splitEmerge(reveal)` is kept as a legacy alias** of `petalEmerge(reveal).scale` — used by the inner geodesic + outer surfaces geodesic where uniform scale is preferred over per-face petal motion.
