...
- **Substrate dock (`.ilayer__brandmark-anchor`, ADR-012 v4)** — the anchor is `position: absolute` at upper-center of `#intelligence-layer`. The choreography contract is unchanged at the JS level: `useSigilChoreography.ts` still resolves it via `intelligenceEl.querySelector(".ilayer__brandmark-anchor")` and parks the actor at its rect at full density via the substrate station. Do not move the anchor outside `#intelligence-layer`.

  **NEW in v4 — anchor RECT is non-stationary across the section.** `useIlayerProgress` writes `--ilayer-anchor-x` / `--ilayer-anchor-y` / `--ilayer-anchor-scale` on the anchor element every scroll frame, descending + scaling it from upper-center toward the encode disc's projected screen rect (sourced from `useIlayerGeomStore`, which the R3F scene populates each frame). Live `getBoundingClientRect()` reads (which the lazy resolver enables) handle this transparently — the actor follows the anchor's descent automatically. Reviewers must NOT "fix" the anchor to a static center, NOT cache its rect mid-section, and NOT remove the descent CSS variables. The substrate → rail handoff fires at the section's exit edge by which point the anchor sits at the encode disc — the actor "lifts off" from inside the encode disc toward the rail dock. If the lift-off reads abrupt, widen the rail-entry trigger window in the choreography hook (do not pin the anchor to neutralise the descent).

  The brandmark also fades out via `--ilayer-brand-opacity` across `[0.45..0.60]` of section progress (the morph crossfade where the SVG mark dissolves AS the encode disc fades in at the same rect). The dock-state opacity gate (`[data-brandmark-mode="particle"][data-brand-svg-dock="substrate"]`) is multiplied by this variable — see `landing.css` for the override rule. Do not change `--ilayer-brand-opacity`'s default to anything other than `0` (resting / pre-park) or `1` (fully visible) without tracing through the gate composition.
