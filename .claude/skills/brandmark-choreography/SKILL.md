...
**Current production substrate (2026-06-07, ADR-018):** the Navigate substrate is the migrated Thoughtform compass — 4 concentric rings (gold/dawn + dash), bearing crosshair + ticks, and an eight-ball horizon / attitude cue (bold equator, faint pitch ladder, cardinal diamonds). `ShellSubstrate.tsx` uses `shellWrapEmerge` so the instrument starts large and contracts inward. Gimbal attitude tilt + slow Z breath replace the earlier Y-spin. `SUBSTRATE_CAGE_RADIUS` is `0.42` (outer ring radius); ring radii are proportional to the opening-beat compass. The gold geodesic icosphere (2026-06-06) is retired. Opening beat (`ThoughtformCompassGate`) now renders nested **square portal loops** instead of circles — same flythrough behaviour.
