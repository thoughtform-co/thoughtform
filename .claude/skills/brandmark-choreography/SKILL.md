...
**Wrap-around revision (2026-06-06, ADR-018 Phase 5):** the substrate cage was replaced with the BRAIN ARTIFACT. First built as a dense point cloud + synapse web; revised the same day to a LOW-POLY MESH on user feedback (the cloud read as busy). `lib/brandmark/sampleBrain.ts` now exports `buildLowPolyBrain` — an icosahedron (detail 1, 80 faces) deformed into a brain (ellipsoid + central fissure + lobing noise); `ShellSubstrate.tsx` renders it as a gold wireframe (`EdgesGeometry`) + faint facet fills (`MeshBasicMaterial`) + small vertex nodes (`Points`). The `brainCloud` shader was removed. The brain emerges via `shellWrapEmerge` (starts at 1.85x and CONTRACTS inward, faded in via a presence ramp) so it wraps the mark from outside like a shell closing — never scaling up from a point at the centre. The dawn inner geodesic was dropped at the same time. The brain folds in via `foldEmerge` (FOLD_OVERSHOOT 1.45x → 1.0x) so it wraps the mark from outside instead of expanding through it. `SUBSTRATE_CAGE_RADIUS` (0.7) is retained as the source-orbit CLEARANCE radius; the brain's max extent is ~0.55. The source orbits + surfaces ports also switched from `petalEmerge` to `foldEmerge`. Phase 4 of the same revision tried a traveling brandmark particle cloud across the whole corridor; it was reverted on user feedback, so the DOM `ProjectedBrandmarkActor` still owns the brandmark across travel and only hands off to `SubstrateMorphCloud` (inside `IntelligenceGate`, at the Intelligence anchor) at the Build morph — the original ADR-017 pattern.
