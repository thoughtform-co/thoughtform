...
**⚠️ Handoff timing revised — "crosshair unfurls into the armillary" (2026-06-24, ADR-023).** The bold, legible mark is the **DOM SVG**; the particle core is a DIM, sphere-merged element that does NOT read as a standalone crosshair. So the SVG now STAYS the front mark for most of the Navigate wrap window while the substrate armillary unfurls FROM its plane (rings start coplanar with the crosshair and rotate open), and the SVG → core handoff happens LATE (`BRANDMARK_CORE_PARTICLE_LAYER_BLEND ≈ 0.82`, cut ≈ `0.90`) — hidden inside the ~80%-formed sphere. `uDepth` is decoupled and ramps EARLY (`BRANDMARK_CORE_DEPTH_START_BLEND ≈ 0.30`, before the cut) while the core is hidden behind the front SVG, so it is already 3D when revealed. `PRODUCTION_BASIS` is restored to `dome-fill` + `dot` (the `edge-lattice`/`voxel` "Dither 3" preset made the bare core a radial dust-burst — see ADR-023 Invariant 12), and `STREAM_VEL_BASE` is dropped to ≈0.10 so the settled mark stays a coherent crosshair. **The paragraph below describes the SUPERSEDED `DOLLY_HOLD_END`-anchored cut+extrude model — read it for the mechanism (z-index drop then `display:none`, never an opacity dissolve), but the live thresholds/anchor/basis are per this note + ADR-023.**
