...
**⚠️ Current handoff model — single-painter async-flow morph (2026-06-24, ADR-023).** The corridor brandmark is now the `BrandmarkPhysicsCore` particle field end-to-end; the DOM SVG stays hidden on the capable path. Runtime evidence showed that any visible SVG opacity fade beside a particle cloud reads as a cross-dissolve. The flat rest state is already a dense particle silhouette (`coverMorphRef = 1`, `uDepth = 0`, full draw density, larger flat dots). As `uDepth` rises, the vertex shader gives each particle a deterministic stagger from seed + polar position, pushes it through a small tangent/asymmetric XY arc, pulls it backward on local Z at the flow midpoint, then settles it onto its sampled domed home inside the sphere. Density relaxes with depth (`depthKeep`) from near-solid 2D particle mark to calmer in-sphere corridor budget. `uStream` / `uGlitch` are gated from depth so they join after the flow starts. Do not reintroduce visible DOM SVG, SVG opacity fade, z-index renderer handoff, `coverMorph` 0→1 as the transition, a separate `reveal` opacity multiplier, uniform slab `pos.z *= uDepth` motion, or low-ignite scatter/gather. The sim remains assembled (`seedAtHome`, `ignite=1`); the render shader owns the flow. **The paragraphs below describe SUPERSEDED `DOLLY_HOLD_END`, renderer-ownership, and cover-in models — read them as history only; the live contract is this async-flow note + ADR-023 Invariant 3.**
