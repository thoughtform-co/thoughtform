...
r"""One slot, N draws. Prompt in, images out, one manifest line per draw.
    ...
    python tools/generate.py --slot H-carafe --prompt "..." --out DIR --n 2 \
        --lane nano,sunburst          # the same prompt on both; --n is per lane
...
`gemini*` speaks generateContent, `gpt*` / `dall*` speak /v1/images, and
`render*` goes to headless Chrome (`render.py`) and needs no key. The lane
...
**What a draw consumed is recorded where the provider reports it** (0.7.0):
`usage` carries input and output tokens when the response has them. It is the
only economic fact the harness could ever know for certain, and the eval log
has asked for the cost of a wave since the first one. What a draw *costs* is a
price per lane in `armada.toml [models.prices]`, because a rate card is a fact
about a lane and not something to infer from a response.
"""
...
import render  # noqa: E402  (the render lane: headless Chrome on the dashboard)
...
def lanes_from(value: str, cfg: dict | None = None) -> list[str]:
    """`--lane` as a list. One name draws on one model; a comma list draws the
    SAME prompt, references and settings on each of them, which is the only way
    a comparison between two models is a comparison rather than two separate
    runs taken on trust. `--n` is per lane, and the lane is already in every
    filename and manifest row, so the draws cannot collide.

    Every name is validated before the first request goes out: finding a typo
    in the third lane after paying for the first two is half a wave lost."""
    cfg = cfg or config.load()
    out: list[str] = []
    for name in (x.strip() for x in str(value).split(",")):
        if name and name not in out:      # a repeated lane is a typo, not two draws
            out.append(name)
    if not out:
        config.die("--lane needs at least one lane name. armada.toml [models.lanes] "
                   "declares: " + ", ".join(cfg["models"]["lanes"]))
    for name in out:
        lane_model(name, cfg)             # dies here, naming the declared lanes
    return out
...
def _usage_from(data, into):
    """Whatever the provider said this call consumed, in one shape. Both APIs
    report it under their own names and neither is obliged to report it at all;
    an absent count is an absent key, never a zero, because a zero is a claim
    that nothing was consumed."""
    if into is None or not isinstance(data, dict):
        return
    u = data.get("usageMetadata") or data.get("usage") or {}
    if not isinstance(u, dict):
        return
    tin = u.get("promptTokenCount") or u.get("input_tokens") or u.get("prompt_tokens")
    tout = (u.get("candidatesTokenCount") or u.get("output_tokens")
            or u.get("completion_tokens") or u.get("totalTokenCount"))
    if tin:
        into["in"] = int(tin)
    if tout:
        into["out"] = int(tout)
...
def _openai_read(req, what, usage=None):
    last = ""
...
def openai_generate(key, prompt, model, size="auto", quality="high", usage=None):
    ...
    return _openai_read(req, model, usage)


def openai_edit(key, prompt, refs, model, size="auto", quality="high", usage=None):
...
def draw(slot, prompt, out_dir, refs, lane="", n=1, ar="4:5", size="2K",
    ...
    if api == "render":
        settings = {"renderer": "chrome-headless", "viewport": render.viewport_of(prompt)}
    else:
    ...
    key = None if api == "render" else require(OPENAI_KEY if api == "openai" else GEMINI_KEY)
    ...
    for d in range(1, n + 1):
        ...
        used: dict = {}
        if api == "render":
            blob, err = render.capture(prompt, model_id, meta)
        elif api == "gemini":
            blob, err = gemini(key, prompt, refs, model_id, ar, size, usage=used)
        elif refs:
        ...
        if used:
            # Only when the provider said so. An absent count is an absent key,
            # never a zero: a zero is a claim that nothing was consumed.
            row["usage"] = used
...
def main() -> None:
    ...
    ap.add_argument("--lane", default=cfg["models"]["default"],
                    metavar="LANE[,LANE...]",
                    help="one lane, or a comma list to draw the same prompt on each "
                         "(--lane nano,sunburst); --n is per lane. Declared here: "
                         + ", ".join(sorted(cfg["models"]["lanes"])))
    ...
    ap.add_argument("--quality", default="high", choices=("low", "medium", "high", "max", "auto"),
                    help="`max` exists only on gpt-image-2.5 and later; an older id "
                         "rejects it and the refusal is recorded per draw")
    ...
    lanes = lanes_from(a.lane, cfg)
    if not a.dry_run:
        print("  lane" + ("s" if len(lanes) > 1 else "") + " "
              + ", ".join(ln + " (" + lane_model(ln, cfg) + ")" for ln in lanes)
              + "  ar " + a.ar)
        for i, r in enumerate(a.ref, 1):
            print("    ref " + str(i) + ". " + os.path.basename(r))
    for ln in lanes:
        draw(slot, prompt, out, a.ref, lane=ln, n=a.n, ar=a.ar, size=a.size,
             quality=a.quality, meta=meta_row(suffix=a.suffix, aspect=a.ar),
             dry_run=a.dry_run)
    if not a.dry_run:
