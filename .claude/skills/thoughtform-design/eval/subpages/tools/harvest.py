r"""The report home. What a ship learned, packaged for the fleet.
    ...
    python tools/harvest.py --repo ../ship --ledger harvest/ledger  bring its rows home
...
where its lessons are written; globs are relative to the repo root:
    ...
    memory        the memory file (0.4.0): its tagged lines - breakthroughs and
                  verdicts - travel first, its ruled-out lines join the ruled-out
                  section. An empty string disables it. A repo that is not a
                  ship (no subjects, no rubric, no waves) reports on this and
                  its changelog alone.
...
that list. A line that starts with `=` is matched case-sensitively, for a
product named with a common word (0.4.1). 2026-09: a note came home naming a person the toml had never heard
...
`--ledger DIR` copies the ship's `evals/ledger.jsonl` to
`DIR/<callsign>.jsonl`, **scrubbed again on the way** (0.7.1). It is already
born scrubbed on the ship, and this is the belt on top of that brace: the rows
are the only structured thing that leaves a ship, so they are scrubbed where
they are written and again where they arrive, and the sweep runs over the
result before anything is opened. A row that needed scrubbing here is a defect
in whatever wrote it, and the copy says so rather than tidying up quietly.
...
class Ship:
    ...
    def __init__(self, root):
        ...
        mem = h.get("memory", "MEMORY.md")
        self.memory = os.path.join(self.root, str(mem)) if mem else None
    ...
    def harness(self):
        ...
        if not os.path.isdir(os.path.join(self.root, "tools")):
            return "no tools aboard"       # a project on the standard, not a ship
    ...
    def memory_since(self, since):
        """The memory file's entries dated on or after `since` (every entry on
        a first harvest), as two lists: the tagged lines (breakthroughs and
        verdicts, the essence the port reads first) and the ruled-out lines.
        A repo with no memory file, or one that disabled it, has none of
        either; that is "None" in the note, never a crash."""
        if not self.memory or not os.path.isfile(self.memory):
            return [], []
        tagged, ruled = [], []
        for e in config.memory_entries(self.memory):
            if e["date"] is None:
                continue
    ...
    def nouns(self):
        """Everything this ship must not send home. `config` owns the rule since
        0.7.0, when the ledger became a second thing that must be born scrubbed:
        two implementations of one rule is a rule that will disagree with
        itself, and the half that disagrees is the half nobody is watching."""
        return config.nouns(self.cfg, self.scrub_file)
...
def _memory_line(e):
    """A memory entry back in its own grammar, so the note and the timeline
    read the same line the ship wrote."""
    return ("- " + e["date"].isoformat()
            + ((" [" + e["tag"] + "]") if e["tag"] else "")
            + " " + e["text"]
            + ((" -> " + e["pointer"]) if e["pointer"] else ""))
...
# `config` owns both since 0.7.0. They stay named here because every ship-side
# and port-side caller has always reached for `harvest.scrub`, and a rename
# would be churn in the one path that must not surprise anyone.
scrub = config.scrub
by_placeholder = config.by_placeholder


# ------------------------------------------------------------------- ledger ---

def carry_ledger(ship, out_dir):
    """Copy the ship's ledger to `<out_dir>/<callsign>.jsonl`, scrubbed again.

    The rows are the only structured thing that leaves a ship, so they are
    scrubbed where they are written and again where they arrive. A hit here is
    a defect in whatever wrote the row, which is why the count is printed
    instead of being quietly cleaned up: the note's de-clienting section reports
    a scrub as normal, and a ledger's reports one as news.

    Returns (rows written, hits by placeholder). No ledger is (0, {}), never an
    error: a ship that has not run a wave since the tool arrived has nothing to
    carry and is not broken."""
    src = os.path.join(ship.root, "evals", "ledger.jsonl")
    ...
    with open(src, encoding="utf-8") as f:
        for line in f:
            ...
            if not s:
                continue
            clean, found = config.scrub(s, nouns)
            for phrase, (n, placeholder) in found.items():
                hits[phrase] = (hits.get(phrase, (0, placeholder))[0] + n, placeholder)
            kept.append(clean)
    os.makedirs(os.path.abspath(out_dir), exist_ok=True)
    dest = os.path.join(out_dir, ship.callsign + ".jsonl")
...
def build(ship, since, apply_scrub, commits_sha=None):
    ...
    tagged, ruled = ship.memory_since(since)
    ...
    # The essence first: what the ship marked as confirmed good, or as a
    # verdict, in its memory file. One line each, in the grammar it was
    # written in, so the port's timeline can carry the same line.
    lines.append("## Breakthroughs")
    lines.append("")
    if not tagged:
        lines.append("None tagged since " + (since.isoformat() if since else "the beginning") + ".")
    for e in tagged:
        lines.append(_memory_line(e))
    lines.append("")
    ...
    for src, head, body, _d in ro:
        ...
        lines.append("")
    if ruled:
        lines.append("### From the memory file")
        lines.append("")
        for e in ruled:
            lines.append(_memory_line(e))
        lines.append("")
    ...
    # A repo whose only news is a tagged memory line has news: without the
    # last two the nightly would exit 2 and skip it.
    nothing_new = not (entries or reps or ro or findings or named or commits or tagged or ruled)
    return text + "\n" + "\n".join(tail) + "\n", hits, nothing_new
...
def main():
    ...
    ap.add_argument("--ledger", default=None, metavar="DIR",
                    help="copy the ship's evals/ledger.jsonl to DIR/<callsign>.jsonl, "
                         "scrubbed again on the way")
    ...
    if a.ledger:
        n, hits = carry_ledger(ship, a.ledger)
        if not n:
            print("  ledger      none aboard " + ship.callsign
                  + " (no evals/ledger.jsonl). Nothing carried.")
            raise SystemExit(EXIT_OK)
        print("  callsign    " + ship.callsign)
        print("  ledger      " + str(n) + " rows -> "
              + os.path.join(a.ledger, ship.callsign + ".jsonl"))
        if hits:
            counts = by_placeholder(hits)
            print("  SCRUBBED    " + ", ".join(k + " x" + str(v)
                                               for k, v in sorted(counts.items()))
                  + " on the way home. A ledger row is meant to be born clean; "
                    "whatever wrote these has a bug.")
        else:
            print("  scrubbed    0 hits; the rows arrived as they were written")
        raise SystemExit(EXIT_OK)
    ...
    tagged, ruled = ship.memory_since(since)
    print("  memory      " + str(len(tagged)) + " tagged, " + str(len(ruled)) + " ruled out"
          + ("" if ship.memory and os.path.isfile(ship.memory) else "  (no memory file)"))
    if a.commits_since:
