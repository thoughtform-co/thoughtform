...
def copy_templates(target: Path, values: dict, src: Path | None = None,
                   skip_existing: bool = False) -> tuple[list[str], list[str]]:
    """Copy a template tree into `target`, substituting the placeholders.
    Returns (written, skipped) as paths relative to the target. `src` is the
    template tree (default: the ship scaffold in `templates/`; the project
    standard lives in `templates-project/`). With `skip_existing`, a file that
    is already there is left alone and reported rather than overwritten -
    that is how a repo that already exists is brought onto the standard."""
    src = Path(src) if src else TEMPLATES
    if not src.is_dir():
        config.die("no " + src.name + "/ beside " + str(HERE) + " — this is not the Armada "
    ...
    for root, dirs, files in os.walk(src):
        ...
        for f in sorted(files):
            ...
            rel_dest = str(dest.relative_to(target)).replace("\\", "/")
            if skip_existing and dest.exists():
                skipped.append(rel_dest)
                continue
            data = path.read_bytes()
            if is_text(data):
                # `newline=""` because the bytes were read raw: the string still
                # holds whatever line endings the template has, and letting the
                # writer translate on top of them turns a CRLF template into
                # \r\r\n. A toml written that way does not parse, and the ship
                # it scaffolds is broken at `doctor.py` rather than here
                # (2026-09, found by the suite the day a template was edited by
                # a script that saved CRLF).
                dest.write_text(substitute(data.decode("utf-8"), values),
                                encoding="utf-8", newline="")
            else:
                shutil.copy2(path, dest)        # byte-identical, always
...
def main():
    ...
    written, _skipped = copy_templates(target, values)
    n_t = len(written)
