...
**Where a slot has more than one draw, one of them is marked best.** That
control is the comparative label, and it is the most reliable judgment on the
page: asked to score pictures, experts disagree with themselves and with each
other; asked to pick the best of a matched set, they agree. A slot with one
draw has nothing to compare, so it gets no control rather than a meaningless
one.

Submit downloads `verdicts-<date>.json`:

    {wave, date, who, verdicts: {<file>: {approved: bool, comment: str}},
     best: {<slot>: <file>}}
...
next session reads it back — `python tools/ledger.py tick --handback <file>`
turns it into rows, and `tools/calibrate.py` turns those rows into the
instrument's agreement with the person.
...
.best-btn{flex:0 0 42px;height:42px;border-radius:9px;border:2px solid #6f7ad9;background:transparent;color:#8d96e4;font:600 12px inherit;cursor:pointer;letter-spacing:.03em}
.best-btn:hover{background:#262a45}
.card.best .best-btn{background:#6f7ad9;color:#fff;border-color:#6f7ad9}
.card.best{border-color:#6f7ad9}
.card.best.ok{border-color:#2f9e63}
...
JS = r"""
const S = {};
const BEST = {};          /* slot -> file. The comparative label. */
let LB = -1;
function refresh(k){
  ...
  c.classList.toggle('best', BEST[s.slot] === s.file);
  ...
  document.getElementById('count').textContent =
    ...
    (CONTESTED ? '   ·   ' + picked + ' / ' + CONTESTED + ' slots have a best' : '');
}
function toggleBest(k){
  const s = S[k];
  /* Clicking the marked one again clears the slot: no best is an honest
     answer, and a control that cannot be unset collects clicks, not judgments. */
  if (BEST[s.slot] === s.file) delete BEST[s.slot];
  else BEST[s.slot] = s.file;
  ORDER.forEach(refresh);
}
function toggleOk(k){
  const s = S[k];
...
function submit(){
  ...
  const payload = JSON.stringify({wave: WAVE, date: stamp, who: who,
                                  verdicts: verdicts, best: BEST}, null, 1);
...
def build(wave_dir: str, out_path: str, title: str) -> str:
    ...
    # Which slots have something to choose between. A slot with one draw gets no
    # best-of-slot control: there is no comparison to make, and a control that
    # can only be clicked one way collects clicks rather than judgments.
    slot_counts: dict[str, int] = {}
    for path, _f in items:
        s = config.slot_of(os.path.basename(path))
        slot_counts[s] = slot_counts.get(s, 0) + 1
    contested = sum(1 for n in slot_counts.values() if n > 1)
    ...
    for folder in sorted(groups):
        ...
        for path in paths:
            ...
            slot = rec.get("slot") or row.get("slot") or config.slot_of(name)
            ...
            cards.append(
                ...
                "'" + key + "')\">&#10003;</button>"
                + ('<button class="best-btn" title="Best draw of this slot" '
                   "onclick=\"toggleBest('" + key + "')\">best</button>"
                   if slot_counts.get(slot, 0) > 1 else "")
    ...
    doc = ('<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">'
           ...
           + "Every grade below is a floor, not a verdict — it is the first pass of "
             ...
             "file."
           + ("<br><b>best</b> marks the best draw of a slot, on the "
              + str(contested) + " slot" + ("" if contested == 1 else "s")
              + " with more than one. That comparison is the most reliable judgment "
                "on this page — picking the best of a matched set is something people "
                "agree on, and scoring pictures one at a time is not."
              if contested else "")
           + "</div>"
           + "".join(sections)
           + '<div class="bar"><span class="count" id="count">0 reviewed</span>'
             '<label class="count">reviewed by '
             '<select id="who"><option value="owner">us</option>'
             '<option value="client">the client</option></select></label>'
             ...
             "<p>Download the file into the wave folder, then run "
             "<code>python tools/ledger.py tick --handback &lt;file&gt;</code>. A comment "
             "is quoted verbatim into the log and decoded into a check; the ticks and "
             "the bests become the labels the grader is measured against."
             '</p><textarea id="json-out" readonly></textarea>'
           ...
           + ";const CONTESTED=" + json.dumps(contested)
...
def main():
    ...
    hb = "verdicts-" + date.today().isoformat() + ".json"
    print("  today's handback would be " + hb)
    print("  then:    python tools/ledger.py tick --handback \"" + hb + "\"")
