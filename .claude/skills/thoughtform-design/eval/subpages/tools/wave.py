...
def main() -> None:
    ...
    ap.add_argument("--lane", default=cfg["models"]["default"],
                    metavar="LANE[,LANE...]",
                    help="one lane, or a comma list to run every slot on each "
    ...
    # Validated before anything is drawn, and the jobs are the product of slots
    # and lanes: one lane is the old behaviour exactly, several run the same
    # prompt on each so the difference between two draws is the model.
    lanes = generate.lanes_from(a.lane, cfg)
    jobs = [(t, s, ln) for (t, s) in todo for ln in lanes]

    print("  wave     " + a.wave)
    print("  setting  " + a.setting + ("  (repair clauses appended)" if a.repair else ""))
    print("  slots    " + str(len(todo)) + " x " + str(a.n) + " draws on "
          + ", ".join(ln + " (" + generate.lane_model(ln, cfg) + ")" for ln in lanes)
          + ("   = " + str(len(jobs) * a.n) + " draws" if len(lanes) > 1 else ""))
    ...
    def one(job):
        t, s, lane = job
        name = t + "-" + s
        # The slot passed to draw() stays the slot; the lane is only a label, so
        # a multi-lane wave reads as a list of models and not of mystery rows.
        label = name + ("  " + lane if len(lanes) > 1 else "")
        try:
            ...
            files = generate.draw(
                ...
                lane=lane, n=a.n, ar=spec["ar"], size="2K",
            ...
            return (label, len(files), None)
        except SystemExit as e:
            return (label, 0, str(e).strip())
            ...
            return (label, 0, type(e).__name__ + ": " + str(e))

    noun = " planned" if a.dry_run else " files"
    t0 = time.time()
    done = 0
    width = 34 if len(lanes) > 1 else 22
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, a.workers)) as ex:
        for name, n, err in ex.map(one, jobs):
            done += 1
            line = ("  [" + str(done).rjust(2) + "/" + str(len(jobs)) + "] "
                    + ("ok  " if err is None else "FAIL") + " " + name.ljust(width)
                    + str(n) + noun)
            if err:
...
def _run_situation(a, cfg: dict) -> None:
    ...
    lanes = generate.lanes_from(a.lane, cfg)
    jobs = [(i, ln) for i in names for ln in lanes]
    print("  interiors " + str(len(names)) + " x " + str(a.n) + " draws on "
          + ", ".join(lanes))
    print("  out       " + str(out_dir) + "\n")

    def one(job):
        interior, lane = job
        label = interior + ("  " + lane if len(lanes) > 1 else "")
        try:
    ...
    width = 32 if len(lanes) > 1 else 20
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, a.workers)) as ex:
        for interior, n, err in ex.map(one, jobs):
            done += 1
            generate.say("  [" + str(done) + "/" + str(len(jobs)) + "] "
                         + ("ok  " if err is None else "FAIL") + " " + interior.ljust(width)
