...
def check_keys(rep, cfg):
    ...
    for lane, model in cfg["models"]["lanes"].items():
        name = key_name_for(model)
        if not name:
            rep.ok("lane " + lane, "a render lane (" + str(model) + "): headless Chrome, no credential")
            continue
        wanted.setdefault(name, []).append(lane)
...
def check_ledger(rep, cfg):
    """The ledger and the prices. Neither breaks a wave, so neither is a failure:
    a ship draws perfectly well while recording nothing about what it learned,
    which is exactly how fourteen of them came to have no record of the person
    at all."""
    prices = dict((cfg.get("models") or {}).get("prices") or {})
    lanes = sorted(cfg["models"]["lanes"])
    priced = [ln for ln in lanes if prices.get(ln)]
    if not lanes:
        pass
    elif not priced:
        rep.warn("lane prices", "none set in [models.prices]. Cost per keeper will "
                 "be blank rather than guessed; a price is a fact about a lane and "
                 "the eval log asks for it every wave.")
    elif len(priced) < len(lanes):
        rep.warn("lane prices", ", ".join(priced) + " priced; no price for "
                 + ", ".join(ln for ln in lanes if ln not in priced))
    else:
        rep.ok("lane prices", str(len(priced)) + " of " + str(len(lanes)) + " lanes")

    p = config.ledger_path()
    if not p.exists():
        rep.warn("ledger", "none yet at " + config.LEDGER_FILE + ". A wave that has "
                 "already run is added with `python tools/ledger.py --rebuild`.")
        return
    rows = config.ledger_rows(p)
    kinds = {}
    for r in rows:
        kinds[r["kind"]] = kinds.get(r["kind"], 0) + 1
    rep.ok("ledger", str(len(rows)) + " rows ("
           + ", ".join(k + " " + str(v) for k, v in sorted(kinds.items())) + ")")
    if not kinds.get("tick"):
        rep.warn("ticks", "no handback has been read. Until one is, the grader is "
                 "measured against nothing, which is what 'reporting only' means. "
                 "Build the gallery, tick it, then `ledger.py tick --handback`.")
