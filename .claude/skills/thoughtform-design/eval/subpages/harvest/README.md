...
The sweep is the last gate, and it runs **at the home port, over what arrived
there** - never here. It greps a directory for this ship's nouns, prints
`path: count` and exits 4 on any hit, and never prints the word it found. This
repo is full of those nouns on purpose, so a sweep of it can only ever be a
long list of true positives:

```bash
python tools/harvest.py --repo <this ship> --sweep .    # from the home port: 0 hits
```
