# ADR-051: Remnant 3D â€” the Gateway key visual rebuilt as procedural geometry

**Date:** 2026-07-26
**Status:** Accepted (lab surface only; not wired into any production route)
**Flag:** none â€” `/test/remnant-3d` is dev-only, blocked in production by `proxy.ts`
**Builds on:** ADR-027 (gateway motion lab â€” the depth/mask derivatives this consumes, and the ~2.5Â° orbit ceiling this is measured against), ADR-023 (the anti-blob, edge-first sampling doctrine), ADR-025 (the services hologram's GLBâ†’point-cloud precedent).
...
The question asked was how far the [img2threejs](https://github.com/hoainho/img2threejs) skill can take the Gateway key visual (`public/images/Gateway_v1b.webp` â€” the eroded ring-and-tail "REMNANT STRUCTURE") toward a 3D asset. Target agreed up front: a **showpiece with a near-fixed camera**, not a corridor fly-past and not a mesh export.
...
Not a library â€” a **Claude Code skill** (Apache-2.0, v1.4.0). Nothing to `npm install`, nothing imported at runtime. It drives an agent through eight vision-gated passes (blockout â†’ structural â†’ form-refinement â†’ material â†’ surface â†’ lighting â†’ interaction â†’ optimization) with hard gates on silhouette IoU, proportion, symmetry, and PBR-inference confidence â‰¥ 0.7, and emits a TypeScript factory returning a `THREE.Group`. Its Python is stdlib-only; it enforces, the model judges.

## Decision â€” the tool owns structure and evidence; we own the form
...
- **Material scalars extracted from real pixels**, not invented. `extract_pbr_evidence.py` returned independent albedo/roughness/height/normal/AO for all four materials at confidence 0.83â€“0.86, and `extract_part_color_recipe.py` produced per-component CIE-Lab colour recipes for all 16 non-material-only components. This is the part that would have been slowest and least reliable by hand.
...
## Decision â€” the form model

The object is ONE ribbon of laminated sheet swept along an inward spiral. It is not a torus and not a comet. Reading the plate: the bright face is the outer surface of a rolled sheet, and the dark concentric striations are that sheet's **cut edge** â€” so the band's width runs along the coil axis and its thickness is radial, exactly like a roll of tape. Along the spar the plies visibly delaminate and shear.

Constants were **measured, not eyeballed** (`remnantSpine.ts`):

- Cavity inscribed radius 104px vs coil outer radius 334px â†’ ratio 0.311, from a distance transform with a 32-ray enclosure test (the mask's ring is broken at the lower left, so a flood-fill hole test finds nothing).
...
| 0Â°       | the plate                                                          |
| 8Â°       | still the plate                                                    |
| 15Â°      | plausible; the coil begins to open                                 |
| 22Â°      | departing â€” the cavity shows an inner wall the plate never shows |
| 30â€“45Â° | a different object: a roll of tape seen obliquely                  |

**Usable range â‰ˆ Â±8Â° comfortably, â‰ˆ Â±15Â° at the limit** â€” against ADR-027's measured ~2.5Â° for the depth-relief treatment on this same plate. Roughly a 6Ã— improvement, and the reason is structural: this is genuinely 3D geometry rather than a displaced plane, so it does not smear, it just starts showing invented surfaces. It never collapses to a degenerate view, which is the gate img2threejs calls "multi-angle or it didn't happen".

## Invariants

1. **The ribbon is built in the coil's own frame (coil axis = +Z), and the view tilt is applied to the whole object as a group rotation.** Baking the tilt into the spine breaks the band's width direction â€” the builder derives width from the coil axis, and a pre-tilted spine makes world +Z something else, yielding a coiled clock spring instead of a roll of tape. This was a real bug; `TILT_EULER` carries the warning.
2. **The lab drives the camera imperatively.** R3F applies `<Canvas camera={...}>` only on first render. Passing a changing prop moves the readout but not the camera, and the orbit sweep then reads "the silhouette never changes" â€” the exact opposite of the truth. This was also a real bug, caught only because a 45Â° orbit looked implausibly stable.
...
5. The fray samples the ribbon's own vertices weighted by `sweepT`, not an `EdgesGeometry` â€” `EdgesGeometry` drops custom attributes, and `sweepT` is precisely the density signal needed.
...
- The reconstruction is auditable and re-runnable â€” spec, evidence and generator output are all checked in.
