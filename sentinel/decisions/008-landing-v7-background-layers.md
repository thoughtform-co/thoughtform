...
| 3     | `.hero`                                                 | relative | 4                         | key visual + overlay; goes `visibility: hidden` at `heroCover ≥ 1`. ⚠ RELATIVE z:4, not sticky z:1 — ADR-022 v8 made the card the MOVER (corrected 2026-08-24, ADR-075) |
...
| 7a | first beat's `.arc-plane` during `html[data-arc-entry]` | fixed | 1 | the HELD panel the arc's curtain lifts off — the plane only, so the stage keeps the beat's flow height and every terminal clock stays measured (ADR-075) |
