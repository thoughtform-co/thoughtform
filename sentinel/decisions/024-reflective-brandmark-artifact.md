...
- Treat scene as the persistent lab stage, not a third brandmark mode. The primary renderer choice is now only `Solid` or `Particle`; both render inside the active scene frame.
- Keep brandmark presets independent from the active stage. Selecting `Active Chamber`, `Terminal Plot`, or any other preset changes material/particle settings, lighting, postprocessing, and motion, but it does not leave the current Services/orbit stage unless the user changes the Stage control explicitly.
- Make the Services rails composition the default scene frame. It must reuse the production Services grammar directly: the real Services orbit map, `ServiceCelestialCard` content, HUD side rails, bottom compass, top-right status label, and centered particle-sun scale. It must not show the reflective lab's generic reticle, oversized red signal rings, or lightformer reflection-card bars in the Services frame.
- Keep the lab control panel reachable over the Services stage so presets can be browsed while the scene stays active. This is a browsing lab view, not a pixel-identical production screenshot mode.
